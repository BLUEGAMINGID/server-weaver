    ...
    >
      ...
      <div className="flex items-center justify-between mb-4 md:mb-6">
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <div>
            <h2 className="font-semibold text-sm md:text-base">{status.name}</h2>
            <div className="flex items-center gap-2 mt-0.5 md:mt-1">
              <span className={`inline-flex items-center gap-1 md:gap-1.5 text-[10px] md:text-xs px-1.5 md:px-2 py-0.5 rounded-full ${getStatusColor()}`}>
                <Circle className="w-1.5 h-1.5 md:w-2 md:h-2 fill-current" />
      ...
      <div className="grid grid-cols-3 gap-2 md:gap-4 mb-4 md:mb-6">
        ...
        <div className="space-y-1 md:space-y-2">
          ...
          <p className="text-sm md:text-lg font-semibold">{status.cpu}%</p>
        ...
        <div className="space-y-1 md:space-y-2">
          <div className="flex items-center gap-1 md:gap-2 text-[10px] md:text-xs text-muted-foreground">
            <MemoryStick className="w-3 h-3 md:w-3.5 md:h-3.5" />
            <span>RAM</span>
          </div>
          <p className="text-sm md:text-lg font-semibold">{formatBytes(status.memory.current)}</p>
          <ProgressBar 
        ...
        <div className="space-y-1 md:space-y-2">
          ...
          <p className="text-sm md:text-lg font-semibold">{formatBytes(status.disk.current)}</p>
          <ProgressBar 
      ...
      {/* Power Controls */}
      <div className="flex gap-2 pt-3 md:pt-4 border-t border-border/50">
        {status.status === 'offline' ? (
          <Button 
            onClick={() => onPowerAction('start')} 
            className="flex-1 h-9 md:h-10 text-xs md:text-sm bg-primary hover:bg-primary/90"
          >
            <Power className="w-3.5 h-3.5 md:w-4 md:h-4 mr-1.5 md:mr-2" />
            Start
          </Button>
        ) : (
          <>
            <Button 
              ...
              className="flex-1 btn-glass h-9 md:h-10 text-xs md:text-sm"
            >
            ...
            <Button 
              ...
              className="flex-1 btn-glass h-9 md:h-10 text-xs md:text-sm"
            >
              <Square className="w-3.5 h-3.5 md:w-4 md:h-4 mr-1 md:mr-2" />
              <span className="hidden xs:inline">Stop</span>
            </Button>
            <Button 
              variant="destructive" 
              onClick={() => onPowerAction('kill')} 
              className="px-3 md:px-4 h-9 md:h-10"
            >
